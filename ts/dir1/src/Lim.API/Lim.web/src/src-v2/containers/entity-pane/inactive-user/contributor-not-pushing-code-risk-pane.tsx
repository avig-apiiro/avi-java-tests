...
import { InactiveEvidenceContributorTab } from '@src-v2/containers/entity-pane/inactive-user/main-tab/inactive-evidence-contributor-tab';
import { FixInactiveAdminCard } from '@src-v2/containers/entity-pane/inactive-user/remediation/fix-inactive-admin-card';
import { useInject } from '@src-v2/hooks';

export function ContributorNotPushingCodeRiskPane(props: RiskPaneProps) {
  const { inventory } = useInject();
  const fetchInventory = useCallback(
    ({ risk }) =>
      inventory.getInactiveRepositoryContributorElement(
  ...
  return (
    <RiskPane {...props} header={<RiskPaneHeader />} elementDataFetcher={fetchInventory}>
      <RiskPaneTabsRouter
        evidence={InactiveEvidenceContributorTab}
